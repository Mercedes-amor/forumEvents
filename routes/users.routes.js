...
// GET "/api/user/userProfile" => pagina personal del usuario
...
// DELETE "/api/user/deleteAcount" => eliminar usuario
...
router.delete("/deleteAcount", isAuthenticated, async (req, res, next) => {
  ...
  try {
    await Session.updateMany(
      {
        $or: [
          { assistants: { $in: req.payload._id } },
          { hostedBy: req.payload._id },
        ],
      },
      {
        $pull: { assistants: req.payload._id },
        $pull: { hostedBy: req.payload._id },
      }
    );
